...
const BLOG_POSTS = [
  {
    ...
    title: "Topic-Based Tech Tool Implementation: Docker",
    date: "May 4, 2018",
    author: "Admin",
    readTime: "6 min read",
    excerpt: "Learn how to use Docker to build, package, and deploy a Node.js web application. We'll explore the setup process, Dockerfiles, and how containerization solves production environment consistency.",
    category: "WordPress",
    tags: ["Docker", "Containerization", "NodeJS", "DevOps"],
    content: `
...
<p>Docker has revolutionized cloud computing, paving the way for orchestration tools like Kubernetes. The current trend focuses heavily on <strong>DevSecOps</strong>—integrating security scanning directly into the Docker build pipeline. Looking forward, the ecosystem is shifting towards lightweight container runtimes (like containerd) and WebAssembly (Wasm) integration, promising even faster startup times and cross-platform execution without traditional OS overhead.</p>
  ...
  {
    id: "port-scanner-analysis",
    title: "Code Break Analysis: Python Network Port Scanner",
    date: "April 28, 2018",
    author: "Admin",
    readTime: "8 min read",
    excerpt: "A deep dive into writing a basic TCP port scanner using Python's socket module. We'll analyze the connection logic, discuss error handling optimizations, and reflect on network reconnaissance.",
    category: "WordPress",
    tags: ["Python", "Networking", "Port Scanning", "Socket"],
    content: `
...
// ==========================================
let state = {
  currentView: 'home', // 'home', 'about', 'contact', or post ID
  searchQuery: '',
  selectedCategory: 'All',
};

// ==========================================
// 3. MAIN APP LAYOUT RENDERING
// ==========================================
function renderApp() {
  const root = document.querySelector('#app');
  if (!root) return;

  // 1. Header Banner exactly like mockup
  const headerHtml = `
  ...
  // 2. Navigation Bar matching mockup
  const navbarHtml = `
    <div class="navbar-wrapper">
      <div class="container navbar">
        <ul class="nav-links">
          <li>
            <a href="#" class="${state.currentView === 'home' ? 'active' : ''}" data-nav="home">
              Home
            </a>
          </li>
          <li><a href="#" class="${state.currentView === 'about' ? 'active' : ''}" data-nav="about">About</a></li>
          <li><a href="#" class="${state.currentView === 'contact' ? 'active' : ''}" data-nav="contact">Contact</a></li>
        </ul>
  ...
  root.innerHTML = headerHtml + navbarHtml + layoutHtml + footerHtml;
  ...
  if (state.currentView === 'home') {
    ...
    renderDetailView(state.currentView);
  }
...
function setupNavEvents() {
  ...
  // Search trigger in header focuses the sidebar search input
  const searchTrigger = document.querySelector('#navbar-search-trigger');
  if (searchTrigger) {
...
// ==========================================
// 5. VIEW 1: MAIN BLOG FEED (HOME)
// ==========================================
function renderHomeView() {
  const container = document.querySelector('#main-content-pane');
  if (!container) return;
  ...
  if (filteredPosts.length === 0) {
    ...
    filteredPosts.forEach(post => {
      feedHtml += `
        <article class="post-card">
          ...
          <h2><a href="#" data-post-link="${post.id}">${post.title}</a></h2>
          <p class="post-excerpt">${post.excerpt}</p>
          <button class="read-more-btn" data-post-btn="${post.id}">
            Read More &rarr;
          </button>
        </article>
      `;
    ...
    // Add Pagination exactly like mockup at the bottom of the feed
    feedHtml += `
  ...
  // Bind Post Click navigation
  container.querySelectorAll('[data-post-link], [data-post-btn]').forEach(elem => {
    elem.addEventListener('click', (e) => {
      e.preventDefault();
      const postId = e.currentTarget.getAttribute('data-post-link') || e.currentTarget.getAttribute('data-post-btn');
      state.currentView = postId;
      renderApp();
      window.scrollTo({ top: 0, behavior: 'smooth' });
    });
  });
  ...
  container.innerHTML = `
    <article class="post-detail-view">
      ...
      <div class="post-meta">
        ...
        <span style="margin: 0 4px;">|</span>
        <span><i class="fa-regular fa-user"></i> ${post.author}</span>
        <span class="tag" style="margin-left: 12px;">${post.category}</span>
      </div>
...
// ==========================================
// 9. SIDEBAR RENDERING WITH MOCKUP WIDGETS
// ==========================================
function renderSidebar() {
  const container = document.querySelector('#sidebar-pane');
  if (!container) return;
  ...
  const searchWidget = `
    <div class="sidebar-widget">
      <form class="search-form" id="search-form">
        <input type="text" class="search-input" placeholder="Search ..." id="search-input" value="${state.searchQuery}">
  ...
  // Widget 2: Recent Posts (Exactly matching the mockup text + links)
  const recentWidget = `
    <div class="sidebar-widget">
      <h3>Recent Posts</h3>
  ...
  // Widget 7: Interactive Sandbox CLI Widget (At the bottom, high value!)
  ...
  container.innerHTML = searchWidget + recentWidget + commentsWidget + archivesWidget + categoriesWidget + metaWidget + terminalWidget;
...
function setupSidebarEvents() {
  ...
  // Recent posts and navigation lists
  document.querySelectorAll('[data-recent-id]').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      const id = e.currentTarget.getAttribute('data-recent-id');
...
// ==========================================
// 10. INTERACTIVE TERMINAL CLI LOGIC
// ==========================================
function setupTerminalLogic() {
  const inputField = document.querySelector('#terminal-input');
  const windowElem = document.querySelector('#terminal-window');
  if (!inputField || !windowElem) return;
  ...
  inputField.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      ...
      printTerminalLine(`guest@theerdha:~$ ${commandText}`, '#cbd5e1');
      processCommand(commandText);
    }
  });

  function printTerminalLine(text, color = '#38bdf8') {
    const line = document.createElement('div');
    line.className = 'terminal-line';
    line.style.color = color;
    line.innerHTML = text;
    windowElem.appendChild(line);
  ...
  function processCommand(cmd) {
    ...
    switch (mainCmd) {
      case 'help':
        printTerminalLine('Active sandbox parameters:', '#a78bfa');
        printTerminalLine('  <span style="color:#e2e8f0">help</span> - Display active lists.');
        printTerminalLine('  <span style="color:#e2e8f0">scan &lt;target&gt;</span> - Trace TCP port connections (Post #2).');
        printTerminalLine('  <span style="color:#e2e8f0">docker build</span> - Assemble app container (Post #1).');
        printTerminalLine('  <span style="color:#e2e8f0">clear</span> - Clear prompt buffer.');
        printTerminalLine('  <span style="color:#e2e8f0">whoami</span> - Check active user context.');
        break;
      ...
      case 'whoami':
        printTerminalLine('guest_user // Session key: WP_GUEST_404', '#10b981');
        break;

      case 'scan':
        if (!arg) {
          printTerminalLine('Usage error: scan &lt;ip/url&gt;', '#f43f5e');
        } else {
          printTerminalLine(`Scanning target ${arg}...`, '#38bdf8');
          printTerminalLine('Scanning common ports [22, 80, 443] at timeout 0.5s...', '#e2e8f0');
          ...
          const interval = setInterval(() => {
            if (idx < ports.length) {
              ...
              let desc = port === 22 ? 'ssh' : port === 80 ? 'http' : 'https';
              printTerminalLine(`Port ${port}/tcp (${desc}) -> <span style="color:#10b981; font-weight:600;">OPEN</span>`, '#10b981');
              idx++;
            } else {
              clearInterval(interval);
              printTerminalLine('TCP connection port scan completed successfully.', '#38bdf8');
            }
          }, 350);
        }
        break;
      ...
      case 'docker':
        if (arg === 'build') {
          printTerminalLine('Building container context: my-node-app...', '#e2e8f0');
          setTimeout(() => printTerminalLine('FROM node:18-alpine -> pull successful', '#38bdf8'), 200);
          setTimeout(() => printTerminalLine('WORKDIR /usr/src/app -> complete', '#e2e8f0'), 500);
          setTimeout(() => printTerminalLine('COPY package*.json ./ -> complete', '#e2e8f0'), 800);
          setTimeout(() => printTerminalLine('RUN npm install -> complete (added 50 packages)', '#e2e8f0'), 1100);
          setTimeout(() => printTerminalLine('COPY . . -> complete', '#e2e8f0'), 1400);
          setTimeout(() => {
            printTerminalLine('Successfully built ad372bdcf361', '#10b981');
            printTerminalLine('Successfully tagged my-node-app:latest', '#10b981');
          }, 1700);
        } else {
          printTerminalLine('Error: Invalid arguments. Try "docker build"', '#f43f5e');
        }
        break;

      default:
        printTerminalLine(`Command not found: '${mainCmd}'. Type 'help' for active commands.`, '#f43f5e');
        break;
    }
  }
}
